...
public class TestZookeeperAuth implements Watcher {
	...
	static void getDataByBadAuthentication() {
		...
		ZooKeeper badzk = null;
		try {
			badzk = new ZooKeeper(CONNECT_ADDR, 2000, null);
			// 授权
			badzk.addAuthInfo(authentication_type, badAuthentication.getBytes());
			Thread.sleep(2000);
			System.out.println(prefix + "获取数据：" + PATH);
			System.out.println(prefix + "成功获取数据：" + new String(badzk.getData(PATH, false, null)));
		} catch (Exception e) {
	...
	static void getDataByNoAuthentication() {
		...
		try {
			System.out.println(prefix + "获取数据：" + PATH);
			nozk = new ZooKeeper(CONNECT_ADDR, 2000, null);
			Thread.sleep(2000);
			System.out.println(prefix + "成功获取数据：" + new String(nozk.getData(PATH, false, null)));
		} catch (Exception e) {
			System.err.println(prefix + "获取数据失败，原因：" + e.getMessage());
		} finally {
			if (nozk != null) {
